...
import 'package:cargo_app/fetches/info_point_fetch.dart';
...
import '../fetches/points_fetch.dart';
...
class AuthClient {
  ...
  Future<bool> postProductPhotoAdd(
    ...
    ImageString.images.add(await ImageString.imageToBase64(file));
    var point = {
      "id": 8.toString(),
      "x": lng.latitude.toString(),
      "y": lng.longitude.toString(),
    };
    FetchClass.jsonPoints["data"].add(point);
    var uri = Uri(
      scheme: 'http',
      host: ip,
      port: port,
...
// final responseData = json.decode();
  ...
  Future<bool> postProductPhotoAddStatic(
      XFile file, LatLng lng, int select, String description) async {
    ImageString.images.add(await ImageString.imageToBase64(file));
    print("\n" + ImageString.images.last);
    print(ImageString.images.length);
    var point = {
      "id": ImageString.images.length,
      "x": lng.latitude.toString(),
      "y": lng.longitude.toString(),
    };
    print(point);
    ...
    print(dataJson);
    InfoPoint.pointsJson.add(dataJson);
    FetchClass.jsonPoints["data"].add(point);
    print(InfoPoint.pointsJson.length);
    return true;
  }
}
